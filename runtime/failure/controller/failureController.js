const failInjectionState = require("../state");
...
function enableFailure(failFlag) {
  if (!validateFailFlag(failFlag)) {
    throw new Error(`Invalid fail flag: ${failFlag}`);
  }

  if (failInjectionState[failFlag]) {
    throw new Error(`${failFlag} already enabled`);
  }
...
function disableFailure(failFlag) {
  ...
  if (!failInjectionState[failFlag]) {
    throw new Error(`${failFlag} already disabled`);
  }
